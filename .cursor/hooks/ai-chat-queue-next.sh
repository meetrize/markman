#!/usr/bin/env bash
# Cursor stop hook: enqueue the next ai-chat step via followup_message.
# Enable/disable in .cursor/ai-chat-queue/state.json ("enabled": true/false).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
STATE="$ROOT/.cursor/ai-chat-queue/state.json"
STEPS="$ROOT/.cursor/ai-chat-queue/steps.json"
...
plan_doc = state.get("planDoc", "docs/ai-chat-hook-steps.zh-CN.md")
...
if next_step > total:
    print(
        json.dumps(
            {
                "followup_message": (
                    f"侧栏 AI 对话开发队列已完成（{total}/{total}）。"
                    "请在 .cursor/ai-chat-queue/state.json 将 enabled 设为 false。"
                    "可选：手动验证 AI Tab、多轮对话与上下文模式，并按 docs/ai-chat-hook-steps.zh-CN.md 建议提交。"
                )
            },
            ensure_ascii=False,
        )
